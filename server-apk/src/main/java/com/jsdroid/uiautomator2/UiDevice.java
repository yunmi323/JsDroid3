 ...
 * @since API Level 16
 */
public class UiDevice implements Searchable, UiAutomation.OnAccessibilityEventListener {
    ...
    private Map<String, UiAutomation.OnAccessibilityEventListener> onAccessibilityEventListenerMap;

    private UiDevice() {
        onAccessibilityEventListenerMap = new HashMap<>();
        try {
            uiAutomation = Nodes.getInstance().getUiAutomation();
    ...
    public void addAccessibilityEventListener(String name, UiAutomation.OnAccessibilityEventListener onAccessibilityEventListener) {
        synchronized (onAccessibilityEventListenerMap) {
            onAccessibilityEventListenerMap.put(name, onAccessibilityEventListener);
        }
    }

    public void removeAccessibilityEventListener(String name) {
        synchronized (onAccessibilityEventListenerMap) {
            onAccessibilityEventListenerMap.remove(name);
        }
    }
    ...
    public void onAccessibilityEvent(AccessibilityEvent accessibilityEvent) {
        synchronized (onAccessibilityEventListenerMap) {
            for (UiAutomation.OnAccessibilityEventListener value : onAccessibilityEventListenerMap.values()) {
                try {
                    value.onAccessibilityEvent(accessibilityEvent);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public List<UiObject2> findObjects(BySelector selector) {
        ...
        for (AccessibilityNodeInfo node : ByMatcher.findMatches(this, selector, getWindowRoots())) {
            ret.add(new UiObject2(this, node));
        }
    ...
    public List<com.jsdroid.uiautomator2.node.Node> getNodes() {
    ...
    public void waitForIdle() {
        long waitForIdleTimeout = Configurator.getInstance().getWaitForIdleTimeout();
        if (waitForIdleTimeout < 1000) {
    ...
    AccessibilityNodeInfo[] getWindowRoots() {
        waitForIdle();
        ArrayList<AccessibilityNodeInfo> rootNodes = new ArrayList<>();
        ...
        return rootNodes.toArray(new AccessibilityNodeInfo[rootNodes.size()]);
    }
    ...
    private boolean uiAutomatorEnable;

    public void connect() {

    }

    public boolean isUiAutomatorEnable() {
        return uiAutomatorEnable;
    }
    ...
    public String executeShellCommand(String command) {
        ...
        for (String line : Shell.SH.run(command)) {
            result.append(line);
            result.append("\n");
        }
    ...
    public String getAct() {
        try {
            String result = executeShellCommand("dumpsys activity activities|grep mResumedActivity");
